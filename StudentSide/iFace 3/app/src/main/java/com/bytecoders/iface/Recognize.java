...
//    private FirebaseAuth mAuth;
    private int count;
    String sub_code,pattern_code,idx;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        pattern_code=getIntent().getStringExtra("pattern_code");
...
//                        faceRecognizer.load(f.getAbsolutePath());
    ...
    private void recognize(opencv_core.Rect dadosFace, Mat grayMat, Mat rgbaMat) {
        ...
        if (prediction == -1 || acceptanceLevel >= ACCEPT_LEVEL) {
            ...
            if(count==3)
            {
                Intent intent = new Intent(getApplicationContext(),Pattern_Recognition.class);
                intent.putExtra("std_obj",s);
                intent.putExtra("sub_code",sub_code);
                intent.putExtra("pattern_code",pattern_code);
